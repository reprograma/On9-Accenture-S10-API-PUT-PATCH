...
const postagemAtualizada = (requisicao, resposta) =>{
  const { descricao } = requisicao.body;
  
  const filtrarPostagemAtualizada = tarefaModel.filter(tarefa => {
    return tarefa.descricao == descricao; 
  })
  
  const index = tarefaModel.indexOf(filtrarPostagemAtualizada);

  tarefaModel[index] = filtrarPostagemAtualizada

  resposta.status(200).json(postagemAtualizada);

}

function atualizarTitulo(requisicao, resposta) {
